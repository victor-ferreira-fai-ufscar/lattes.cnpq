...
async def scrape_individual(request: ScrapeRequest):
    ...
    try:
        ...
        if not dados:
            raise HTTPException(
                status_code=500, detail="A IA retornou um resultado vazio."
            )
        ...
        return ScrapeResponse(nome=request.nome, dados=dados, docx_path=docx_path)
...
async def scrape_batch(request: BatchScrapeRequest, background_tasks: BackgroundTasks):
    ...
    for nome in request.nomes:
        try:
            ...
            results.append(ScrapeResponse(nome=nome, erro=str(e)))
