...
- [ ] Melhorar as regras de ESLint do Next.js <https://nextjs.org/docs/app/api-reference/config/eslint> verificar se estou usando as melhores práticas e etc. Precisamos que esse projeto perdure por muitos anos, pq será usado em produção, então é importante manter um código limpo e organizado para facilitar a manutenção e evolução futura do projeto. (Revisar as regras de ESLint do Next.js para garantir que o código esteja seguindo as melhores práticas e padrões recomendados, melhorando a qualidade do código e facilitando a manutenção e evolução futura do projeto.)
- [x] Adicionar os avisos e notificações de forma centralizada que atualmente estão em baixo do "Limpar histórico" e "Escolha o fluxo", para mostrar os avisos e notificações de forma mais organizada e centralizada, facilitando a visualização das mensagens importantes para o usuário. (Adicionar um sistema de notificações centralizado para exibir avisos e mensagens importantes para o usuário, melhorando a comunicação e a experiência geral da aplicação.)
- [x] Adicionar mais nomes aleatórios no placeholder do input de busca, para deixar mais variado e interessante. (Adicionar uma lista de nomes aleatórios no placeholder do input de busca para tornar a interface mais dinâmica e interessante, incentivando os usuários a experimentarem diferentes pesquisas e explorarem as funcionalidades da aplicação.)
